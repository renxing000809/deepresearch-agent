...
<strong>构建一个能够自动化执行深度研究任务的智能体助手。</strong>
...
如表 1.1 所示，三个 Agent 分别负责规划、总结和报告生成。

<div align="center">
  <p>表 1.1 三个 Agent 的职责划分</p>
...
这里的 ToolAwareSimpleAgent 是根据 SimpleAgent 拓展实现，可以在 1.3.2 了解，这里不用深究。
...
### 1.3.2 ToolAwareSimpleAgent 的设计
...
如表 1.2 所示，这次增加的搜索引擎有不同的特点和适用场景。
...
当 Agent 需要调用工具时，它会生成工具调用指令，如图 1.7 所示。
